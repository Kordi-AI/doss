...
`peers/`, and `notes/` are omitted. A host should generate a fresh view after
changing `policy.yaml` or `local/access.yaml`, then give the external-facing
agent only that view instead of the raw vault.
