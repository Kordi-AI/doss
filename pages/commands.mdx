...
| `doss devices` | List synced device registrations, marking current device and active vs. unregistered status. `unregister DEVICE_ID` revokes the recorded GitHub deploy key when present, then sets another non-current device to `unregistered`. |
...
<Callout type="warning">
  `doss devices unregister DEVICE_ID` blocks future Doss-managed GitHub sync when
  the device has recorded deploy-key metadata. It cannot erase an already cloned
  local plaintext vault, and it cannot revoke separate owner account credentials
