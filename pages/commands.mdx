...
| --- | --- |
| `doss init` | Create a new vault, or attach this device to an existing cloud vault. Auto-registers the device and runs `connect` unless `--no-connect` is passed. |
...
| `doss devices` | List synced device registrations, marking current device and active vs. unregistered status. `unregister DEVICE_ID` marks another registered non-current device unregistered. |
| `doss tidy` | Print stale facts, unconfirmed guesses, missing rough values, and notes backlog. Read-only. |
...
<Callout type="warning">
  `doss devices unregister DEVICE_ID` does not revoke device access. It only
  writes `status: unregistered` to the synced device registry. A lost device with
  an existing clone and credentials can still pull or push until you revoke its
  GitHub, SSH, token, or deploy-key access outside Doss.
</Callout>
...
- **Owners**: `init`, `devices`, `doctor`, `tidy`, `uninstall`, and policy edits.
...
| `rough` | string | required under `self/`; owner-authored coarse value for rough disclosure |
