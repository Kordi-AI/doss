...
with `status: active` or `status: unregistered`. `doss init` registers the current
device. `doss init --from owner/repo` registers the new device after clone and
uploads that registration.

<Callout type="warning">
  Device unregister is not revocation. `doss devices unregister DEVICE_ID`
  records that another device should be considered inactive by the vault, but it
  cannot revoke a local clone, SSH key, GitHub token, deploy key, or other git
  credential. For a lost or compromised device, revoke external access outside
  Doss too.
</Callout>

`doss uninstall` is stronger only for the current machine: when safe, it marks
this device `unregistered`, pushes that state, unwires agents, and deletes the
local vault. It still does not delete the cloud copy.
...
| Device is removed normally | `doss uninstall` pushes `status: unregistered`, then deletes the local vault |
| Device is lost or compromised | run `doss devices unregister DEVICE_ID` for bookkeeping and revoke external git credentials outside Doss |
